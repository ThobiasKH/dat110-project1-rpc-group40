	...
	public void run() {
		...
		while (!stop) {
		   ...
		   // if (true)
            ...
            RPCRemoteImpl methodImpl = services.get(rpcid);
            byte[] returnValue = methodImpl.invoke(param);
            byte[] response = RPCUtils.encapsulate(rpcid, returnValue);

            replymsg = new Message(response);
